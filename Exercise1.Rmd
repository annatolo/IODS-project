...
```{r}
# Filtering to see only European countries
gapdata2007_europe <- gapdata2007 %>%
  filter(continent == "Europe")

gapdata2007_europe %>%
  ggplot(aes(x = fct_reorder(country, lifeExp), y = lifeExp)) +
  geom_bar(stat = "identity", fill = NA, color = "deepskyblue", width = 1) +
  coord_flip() +
  scale_y_continuous(breaks = seq(0, 80, 20))
  theme(axis.text.y = element_text(size = 6)) #To make the country names appear less crammed
```
```{r}
# Here's the actual solution to the exercise above
library(tidyverse)
library(gapminder)
